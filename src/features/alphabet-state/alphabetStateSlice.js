...
export const alphabetStateSlice = createSlice({
    ...
    reducers: {
        changeLetterState: (state, action) => {
            ...
            if (!validLetter) {
                state[character]['state'] = "invalid";
                state[character]['possibleIndices'] = [];
            } else {
                state[character]['state'] = "valid";

                if (validPosition) {
                    // We know that the letter is definitively at the given index
                    state[character]['possibleIndices'][index] = true;

                    // We need to remove the index as a possible index from every other letter in the alphabet state
                    Object.keys(state).forEach((key) => {
                        if (key !== character) {
                            delete state[key]['possibleIndices'][index];
                        }
                    })
                } else {
                    // We know the letter is valid but is definitely not at the current index, so we can remove this index as a possibility
                    delete state[character]['possibleIndices'][index];

                    // Check if the length of the possible indices is only one, then we know we've found the exact position
                    if (Object.keys(state[character]['possibleIndices']).length === 1) {
                        state[character]['possibleIndices'][Object.keys(state[character]['possibleIndices'])[0]] = true;

                        // We need to remove the index as a possible index from every other letter in the alphabet state
                        Object.keys(state).forEach((key) => {
                            if (key !== character) {
                                delete state[key]['possibleIndices'][Object.keys(state[character]['possibleIndices'])[0]];
                            }
                        })
                    }
                }
...
export const { changeLetterState } = alphabetStateSlice.actions;
